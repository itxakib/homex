...
const ServiceProviderLogin = () => {
  ...
  try {
    ...
    if (isLogin) {
      navigation.navigate('cnic_verifciation', { cnic: formData.cnic, name: formData.name });
    } else {
      navigation.navigate('cnic_verifciation', { cnic: formData.cnic, name: formData.name });
    }
  } catch (error) {
    setFormErrors({ ...formErrors, general: error.message || 'An error occurred. Please try again.' });
  } finally {
    setIsLoading(false);
  }
};
